...
3. **Radar do EDHREC**: rode `get_edhrec_recommendations` com o nome do comandante (`limit` 15–20) e grave o retorno na seção `Radar do EDHREC` da saída — **você é quem faz esta chamada pela rodada inteira**; draw, ramp, interação e manabase leem a seção deles no seu arquivo. Das seções de tema (`High Synergy Cards`, `New Cards`, `Top Cards`, `Creatures`, `Enchantments`) saem candidatas para o pool. O EDHREC é **fonte de candidatas, não de veredito** — regras em "Fontes de meta" do guia de busca.
...
4. **Radar do EDHREC**: rode `get_edhrec_recommendations` como no build, grave na seção `Radar do EDHREC` e compare com a decklist para achar lacunas — peças de alta sinergia ausentes do deck. Toda sugestão vinda daí passa pelo mesmo filtro das outras (ver "Fontes de meta" no guia de busca).
...
As regras estão em **"Fontes de meta"** do `references/scryfall-search-guide.md` — leia antes da chamada. Em resumo: carta do EDHREC passa pelo mesmo filtro de qualquer outra (oracle, ficha, 2+ sinergias suas); a nota `Synergy` vale e a inclusão vem quebrada; preço do retorno é ignorado; `Game Changers` são marcadas para a cota do bracket; a origem vai escrita na tabela (`origem: EDHREC alta sinergia`). `get_edhrec_combos` está quebrada (HTTP 403) — não chame.
...
## Radar do EDHREC
Consultado em <data>, `limit` <N>. Só nome e nota de sinergia — inclusão vem quebrada e não se copia.
Lido pelas fases seguintes: draw/interação (`Instants`, `Sorceries`, `Utility Artifacts`), ramp (`Mana Artifacts`),
manabase (`Lands`, `Utility Lands`), orquestrador/wincons (`Game Changers`).
### <Seção do retorno>
